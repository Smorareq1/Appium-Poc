...
def pulsar_boton_central_nav(driver):
    ...
    try:
        ...
        if not boton_central:
            ...
            print(" Usando fallback: primer elemento sin content-desc")
...
def llenar_formulario_direccion(driver, campo1="", campo2="", campo3="", campo4="", campo5="", timeout=10):
    ...
    try:
        ...
        if len(campos_edittext) != 5:
            print(f" Esperaba 5 campos, encontré {len(campos_edittext)}")
        ...
        for i, (campo, valor, nombre) in enumerate(zip(campos_edittext, valores_campos, nombres_campos)):
            if valor:
                try:
                    ...
                    print(f" Error llenando {nombre}: {e}")
            else:
                print(f" {nombre}: vacío, saltando...")
...
def eliminar_gestiones_extras_iterativo(driver, max_intentos):
    ...
    """
    print(" Eliminando gestiones extras iterativamente...")
    ...
    while intentos < max_intentos:
        try:
            ...
            if len(imageviews) < 2:
                print(f" {gestion_desc}: No tiene suficientes ImageViews, saltando...")
                intentos += 1
                continue
            ...
            print(f" Elemento no encontrado en intento {intentos + 1}: {e}")
            break
        except Exception as e:
...
def buscar_cliente(driver, texto_busqueda, timeout=10):
    ...
    try:
        ...
        try:
            ...
            print(f" No se encontraron resultados para '{texto_busqueda}'")
            return None
...
def buscar_y_seleccionar_cliente(driver, texto_busqueda, timeout=10):
    ...
    try:
        ...
        if resultado:
            ...
            print(" Haciendo click en el cliente encontrado...")
            resultado.click()
            time.sleep(1)
